...
const LANGUAGE_MAP = {
  ...
  cpp: 'c++', c: 'c', rust: 'rust', ruby: 'ruby',
  sqlite: 'sqlite3', go: 'go', php: 'php', kotlin: 'kotlin',
  swift: 'swift', scala: 'scala', dart: 'dart', perl: 'perl',
  r: 'rscript', elixir: 'elixir', cobol: 'cobol', nasm: 'nasm',
  powershell: 'pwsh', bash: 'bash',
};

// --- Security: Block dangerous commands in shell languages ---
const BLOCKED_SHELL_PATTERNS = [
  /\brm\s+(-\w+\s+)*\//i, /\brm\s+-rf?\s/i,
  /\bcurl\b/i, /\bwget\b/i, /\bnc\b/i, /\bssh\b/i, /\bscp\b/i,
  /\bchmod\b/i, /\bchown\b/i, /\bmkfs\b/i, /\bdd\b.*\bof=/i,
  /\b(shutdown|reboot|halt|init)\b/i, /\bkill(all)?\s/i,
  /\benv\b/i, /\bexport\b/i, /\/etc\/(passwd|shadow|hosts)/i,
  /\/proc\//i, /\bsudo\b/i, /\bsu\b\s/i, /\bsystemctl\b/i,
  /\bapt(-get)?\b/i, /\byum\b/i, /\bdnf\b/i,
  /\bmount\b/i, /\bumount\b/i,
];

function isShellLanguage(lang) { return lang === 'bash' || lang === 'powershell'; }
function containsDangerousCommands(code) { return BLOCKED_SHELL_PATTERNS.some(p => p.test(code)); }
    ...
    async (req, res) => {
        ...
        try {
            ...
            const newChallenge = new Challenge({
                ...
                difficulty,
                score,
                tags: Array.isArray(tags) ? tags.filter(t => t && t.trim() !== '') : (typeof tags === 'string' ? tags.split(',').map(t => t.trim()).filter(t => t) : []),
                solution,
                solutionLanguage,
                testCases,
                author: req.user.id,
...
const deepPopulateComments = async (challengeId) => {
    ...
    const collectAuthorIds = (comments) => {
        ...
        for (const c of comments) {
            ...
            if (c.awards) c.awards.forEach(a => { if (a.user) ids.add(a.user.toString()); });
    ...
    const User = require('../models/User');
    const users = await User.find({ _id: { $in: [...authorIds] } }).select('username profilePictureUrl');
    const userMap = {};
    users.forEach(u => { userMap[u._id.toString()] = { _id: u._id, username: u.username, profilePictureUrl: u.profilePictureUrl }; });
