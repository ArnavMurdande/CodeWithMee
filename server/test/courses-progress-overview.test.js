...
function mockQuery(sql, params, courses, versions, modules, contents, enrollments, progressMap) {
  ...
  if (norm.includes('GROUP BY lp.status')) {
    ...
    const rows = [];
    if (completedCount > 0) rows.push({ status: 'COMPLETED', count: completedCount });
    if (inProgressCount > 0) rows.push({ status: 'IN_PROGRESS', count: inProgressCount });
...
test('P1C-S3: getCourseProgressOverview calculates completed count, percentage, and updates course completion', async () => {
    ...
    modules: [
      {
        title: 'Module 1',
        contents: [
          { kind: 'VIDEO', title: 'Video 1', legacyUrl: 'http://v1' },
          { kind: 'ARTICLE', title: 'Article 1', body: 'Read me' },
        ],
      },
    ],
  });

  const content1 = course.modules[0].contents[0].id;
  await service.enrollInCourse(userId, course.id);

  // Update progress
  await service.updateLessonProgress(userId, course.id, content1, { lastPositionSec: 100 });

  const overview = await service.getCourseProgressOverview(userId, course.id);
  assert.ok(overview.enrollmentId);
  assert.equal(overview.status, 'ENROLLED');
  assert.ok(Array.isArray(overview.breakdown));
});
