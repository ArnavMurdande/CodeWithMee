...
test('P1A-S2: getChallengeForLearner redacts reference solution and hidden test cases', async () => {
  ...
  const created = await service.createChallenge(authorId, {
    ...
    referenceSolution: 'def two_sum(): pass # SECRET SOLUTION',
    starterTemplates: { python: 'def two_sum(nums, target):\n    pass' },
    visibleTestCases: [{ input: '[2,7,11,15], 9', expectedOutput: '[0,1]' }],
    hiddenTestCases: [{ input: '[3,3], 6', expectedOutput: '[0,1]' }],
  });

  const dto = await service.getChallengeForLearner(created.id);

  assert.equal(dto.id, created.id);
  assert.equal(dto.title, 'Two Sum');
  assert.equal(dto.referenceSolution, undefined);
  assert.equal(dto.testCases.length, 1);
  assert.equal(dto.testCases[0].expectedOutput, '[0,1]');
});
