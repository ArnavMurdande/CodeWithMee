...
const Comment = ({ comment, challengeId, token, onAction, depth = 0 }) => {
    ...
    const handleAward = async (awardType) => {
        try {
            await axios.post(
                `http://localhost:5001/api/challenges/${challengeId}/comments/${comment._id}/award`,
                { awardType },
                { headers: { 'x-auth-token': token } }
            );
    ...
    const handleDelete = async () => {
        ...
        try {
            await axios.delete(
                `http://localhost:5001/api/challenges/${challengeId}/comments/${comment._id}`,
                { headers: { 'x-auth-token': token } }
            );
            onAction();
        } catch (error) {
    ...
    return (
        <div className={`reddit-comment ${depth > 0 ? 'nested' : ''}`} id={`comment-${comment._id}`}>
            ...
            <div className="comment-left-col">
                {profilePic ? (
                    <img src={`http://localhost:5001${profilePic}`} alt="" className="comment-avatar" />
                ) : (
                    <div className="comment-avatar-placeholder">{authorInitial}</div>
                )}
                {comment.replies?.length > 0 && !collapsed && (
                    <div className="thread-line-container" onClick={() => setCollapsed(!collapsed)}>
                    ...
                    {Object.keys(awardCounts).length > 0 && (
                        <span className="comment-awards-inline">
                            {Object.entries(awardCounts).map(([type, count]) => (
                                <span key={type} className="award-badge" title={`${awardIcons[type]?.label} x${count}`}>
                                    {awardIcons[type]?.icon}{count > 1 && <span className="award-count">{count}</span>}
                                </span>
                            ))}
                        </span>
                    )}
                ...
                {!collapsed ? (
                    <>
                        ...
                        <div className="comment-action-bar">
                            <button className="action-btn collapse-btn" onClick={() => setCollapsed(true)} title="Collapse thread">
                            ...
                            <button className={`action-btn vote-up ${hasUpvoted ? 'voted-up' : ''}`} onClick={() => handleVote(true)} aria-label="Upvote">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round"><path d="M12 5l-7 7h4.5v7h5v-7H19z"/></svg>
                            </button>
                            <span className={`vote-count ${netScore > 0 ? 'positive' : netScore < 0 ? 'negative' : ''}`}>{netScore}</span>
                            <button className={`action-btn vote-down ${hasDownvoted ? 'voted-down' : ''}`} onClick={() => handleVote(false)} aria-label="Downvote">
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinejoin="round"><path d="M12 19l7-7h-4.5V5h-5v7H5z"/></svg>
                            </button>
                            ...
                            <button className="action-btn" onClick={() => setShowAwardPicker(!showAwardPicker)}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><circle cx="12" cy="8" r="6"/><path d="M8.5 14L7 22l5-3 5 3-1.5-8"/></svg>
                                Award
                            </button>
                            <button className="action-btn" onClick={handleShare}>
                                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/></svg>
                                Share
                            </button>
                            {isAuthor && (
                                <>
                                    <div className="action-spacer" />
                                    <button className="action-btn delete-btn" onClick={handleDelete} title="Delete comment">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M3 6h18"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6"/><path d="M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2"/></svg>
                                        Delete
                                    </button>
                                </>
                            )}
                        ...
                        {showAwardPicker && (
                            <div className="award-picker">
                                {Object.entries(awardIcons).map(([type, { icon, label }]) => (
                                    <button key={type} onClick={() => handleAward(type)} title={label} className="award-option">
                        ...
                        {showReply && (
                            <form onSubmit={handleReplySubmit} className="reply-form">
                                ...
                                <div className="reply-form-actions">
                                    <button type="button" className="cancel-btn" onClick={() => setShowReply(false)}>Cancel</button>
                                    <button type="submit" className="submit-reply-btn" disabled={!replyText.trim()}>Reply</button>
                                </div>
                            </form>
                        )}
                        ...
                        {comment.replies && comment.replies.length > 0 && (
                            <div className="comment-replies">
                                {comment.replies.map(reply => (
                                    <Comment key={reply._id} comment={reply} challengeId={challengeId} token={token} onAction={onAction} depth={depth + 1} />
                                ))}
                            </div>
                        )}
                    </>
                    ...
                    <button className="collapsed-indicator" onClick={() => setCollapsed(false)}>
                        ...
                        {comment.replies?.length || 0} more {comment.replies?.length === 1 ? 'reply' : 'replies'}
                    </button>
                )}
            </div>
        </div>
